...
Option 1 (recommended): use the test bar properly
...
Practical way to set E
	...
	3.	Record the machine Z at that plane: call it Z_plane.
	4.	Set E = Z_plane - F.

This makes:
	•	E + F = Z_plane (a real physical plane)

Option 2: skip meaningful Z calibration (not ideal)

If you don’t have test bar length or a real stack:
	•	You can still validate XY center-find and radius plausibility.
	•	Expect Z probing may alarm 921 or produce unreliable #541.

⸻

B4 Run Procedure (step-by-step)
...
Meaning:
	•	The probing move did not hit anything within the allowed stroke.
...
Meaning:
	•	Z limits are not set (#524/#525 = 0) or are inconsistent (min/max inverted).

Actions:
	•	Verify #524 and #525 are set correctly (mm values).
	•	Confirm conversion logic expectations (library uses #510 to interpret unit mode).
...
Actions:
	•	Run: G65 P9803 A<safe_z_machine_in> to set safe Z.

⸻

Post-run documentation (operator notes)

Record for each successful B4 run:
	•	Date/time
	•	Ring ID B used
	•	E and F used
	•	Result values:
...
Notes / process recommendations
	•	First validation: keep feed override low and use Single Block.
	•	Always start near center to reduce “no trigger” failures.
	•	If you see repeated 921 alarms during XY touches, the approach offset/stroke may need adjustment by engineering.
